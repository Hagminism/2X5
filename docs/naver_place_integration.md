...
3. **지역 중복 방지**: 이전 크롤 bbox와 겹침 비율(분모=현재 bbox) **≥ 90%** 이면 카카오·네이버 **skip**, Supabase bbox 조회만. **&lt; 90%** 이면 API 1세트, **DB에 없는 매장만** 저장. 직전 API 검색에서 **신규 0건**이었고 같은 구역(겹침 ≥90%)이면 **카카오도 skip**.
4. **DB-only 재검색**: bbox 조회 결과 매장 id 목록이 이전과 같으면 **마커 재생성 skip**.
5. **재검색 쿨다운 3초**: 「현 지도에서 검색」 버튼만 회색 비활성. 로딩은 검색·마커 반영 **완료 후** 해제.
