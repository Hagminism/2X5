...
class OnBoardingCustomerViewModel extends ChangeNotifier {
  final AuthRepository _authRepository;
  final UserType _userType;

  OnBoardingCustomerViewModel({
    required AuthRepository authRepository,
    required UserType userType,
  }) : _authRepository = authRepository,
       _userType = userType;

  OnBoardingCustomerState _state = const OnBoardingCustomerState();

  OnBoardingCustomerState get state => _state;

  final StreamController<OnBoardingCustomerEvent> _eventController =
      StreamController<OnBoardingCustomerEvent>();

  Stream<OnBoardingCustomerEvent> get eventStream => _eventController.stream;

  Future<void> onAction(OnBoardingCustomerAction action) async {
    switch (action) {
      case TapBackButton():
        break;
  ...
  Future<void> _submit() async {
    ...
    if (validationErrorMessage != null) {
      _eventController.add(
        OnBoardingCustomerEvent.showSignUpError(validationErrorMessage),
      );
      return;
    }
    ...
    try {
      ...
      _eventController.add(
        OnBoardingCustomerEvent.showSignUpError(_mapFirebaseAuthError(e.code)),
      );
    } catch (e) {
      _eventController.add(
        const OnBoardingCustomerEvent.showSignUpError('회원가입 중 오류가 발생했습니다.'),
      );
    } finally {
