  ...
  static ReviewAiSummary generate({
    ...
    for (final review in reviews) {
      ...
      if (visitPurpose != null && visitPurpose.isNotEmpty) {
        ...
        keywordScores[mappedPurpose] = (keywordScores[mappedPurpose] ?? 0) + 2;
      }

      for (final entry in _keywordRules.entries) {
        for (final keyword in entry.value) {
          if (normalized.contains(keyword)) {
            keywordScores[entry.key] = (keywordScores[entry.key] ?? 0) + 1;
          }
        }
      }
    }
    ...
    final averageRating = ratingTotal / reviews.length;
