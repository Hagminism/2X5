...
class ReviewAiSummaryGenerator {
  ...
  static ReviewAiSummary generate({
    ...
    List<Map<String, dynamic>> naverReviews = const [],
    List<GooglePlaceReview> googleReviews = const [],
  }) {
    final internalCount =
        reviews.where((review) => review.content.trim().isNotEmpty).length;
    final naverCount = naverReviews
        .where((review) => (review['body'] as String? ?? '').trim().isNotEmpty)
        .length;
    final googleCount =
        googleReviews.where((review) => review.text.trim().isNotEmpty).length;

    final reviewCounts = ReviewSummaryCounts(
    ...
    for (final review in reviews) {
      final normalized = review.content.trim().toLowerCase();
      ...
      _analyzeReviewText(
        ...
        positiveSignals: (value) => positiveSignals += value,
        negativeSignals: (value) => negativeSignals += value,
      );
    ...
    for (final review in naverReviews) {
      final normalized = (review['body'] as String? ?? '').trim().toLowerCase();
      if (normalized.isEmpty) {
        continue;
      }
      _analyzeReviewText(
    ...
    for (final review in googleReviews) {
      final normalized = review.text.trim().toLowerCase();
      if (normalized.isEmpty) {
        continue;
      }
  ...
  static void _analyzeReviewText({
    required String normalized,
    required Map<String, double> keywordScores,
    required void Function(double value) positiveSignals,
    required void Function(double value) negativeSignals,
  }) {
